...
function mapAd(r: Row): AdCardData {
  ...
  return {
    ...
    isUrgent: !!(r.is_urgent ?? (r.tipo === "preciso")),
    status: r.status,
...
function listAdsDemo(filters: AdFilters): {
  ...
  const paged = sorted.slice((page - 1) * limit, page * limit).map((a) => {
    ...
    const fromAdImages = db.adImages
        .filter((i) => i.adId === a.id)
        .sort((x, y) => x.ordem - y.ordem)
        .map((i) => i.imageUrl);
    const fromAdField = Array.isArray(adAny.images) ? adAny.images : [];
    const images = fromAdImages.length > 0 ? fromAdImages : fromAdField;
    return {
      id: a.id,
      userId: a.userId,
      tipo: a.tipo,
      ...
      isUrgent: !!(adAny.isUrgent ?? (a.tipo === "preciso")),
      status: a.status,
...
export async function getAdById(id: string): Promise<AdDetail | null> {
  ...
  return {
    ...
    topFeedUntil: adAny.topFeedUntil ?? null,
    isUrgent: !!(adAny.isUrgent ?? (ad.tipo === "preciso")),
    status: ad.status,
...
export async function updateAd(id: string, input: AdInput): Promise<void> {
  ...
  // [SEC-FIX] CWE-639: Em demo, verifica ownership via sessão atual
  const sid = getDemoSessionId();
  ...
  return db.ads
    ...
    .map((a) => {
      ...
      const fromAdImages = db.adImages
          ...
          .sort((x, y) => x.ordem - y.ordem)
          .map((i) => i.imageUrl);
      const fromAdField = Array.isArray(anyA.images) ? anyA.images : [];
      const images = fromAdImages.length > 0 ? fromAdImages : fromAdField;
      return {
        id: a.id,
        tipo: a.tipo,
